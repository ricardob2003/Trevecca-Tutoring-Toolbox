...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.get("/microsoft/start", async (_request, reply) => {
    return reply.code(501).send({
      enabled: false,
      message: "Microsoft Entra ID login is planned but not configured yet.",
    });
  });
  ...
  app.get("/microsoft/callback", async (_request, reply) => {
    return reply.code(501).send({
      enabled: false,
      message: "Microsoft callback endpoint placeholder. Configure Entra ID to enable this flow.",
    });
  });
}